...
const AppliedJob = () => {
    ...
    const remoteJobHandler = () => {
        const addRemoteJobs = saveJobs.filter(saveJob => saveJob.jobType == 'Remote');
        setApplyJobs(addRemoteJobs);
    }

    const onsiteJobHandler = () => {
        const addOnsiteJobs = saveJobs.filter(saveJob => saveJob.jobType == 'Onsite');
    ...
    return (
        <div>
            ...
            <section className='job-des2 flex gap-10 justify-end'>
                <p className='font-bold text-2xl text-white'><button onClick={remoteJobHandler} className='see-all-btn1 p-3'>Remote</button></p>
                <p className='font-bold text-2xl text-white'><button onClick={onsiteJobHandler} className='see-all-btn1 p-3'>Onsite</button></p>
            </section>
            <section className='mt-10 flex flex-col justify-center items-center'>
                <div>
                    {
                        applyJobs.map(applyJob => <Myjob
                            key={applyJob.id}
                            applyJob={applyJob}
                        ></Myjob>)
                    }
